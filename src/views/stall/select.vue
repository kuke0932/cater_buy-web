    ...
    <view-box ref="viewBox">
              ...
              <scroller lock-x height="-140"  @on-scroll="onScroll" ref="scrollerEvent">
                <div class="infor_list">
                  <div class="infor_item" v-for="(item,index) in foodsList" v-bind:key="index">
                    ...
                    <p>计量单位：{{item.unit}}</p>
                    <div class="plusIcon"  @click="clickFoodItem($event)"><img src="../../assets/icons/plus02.svg" width="16" alt=""></div>
                  </div>
                </div>
              </scroller>
            </div>
      ...
      <flexbox :gutter="0" class="btn_box" id="btn_box">
        <flexbox-item :span="5" class="box_left">
          <p @click="showPopup">已选 {{count}} 种</p>
        </flexbox-item>
      ...
      <div v-transfer-dom style="bottom:42px">
        <popup v-model="showPop" style="margin-bottom: 42px">
          <div class="popup">
            <group title="采购明细">
              <flexbox class="details">
                <flexbox-item class="details_right">
                  <span>面粉 * 2</span>
                  <span style="float: right;">&yen200</span>
                </flexbox-item>
              </flexbox>
              <flexbox class="details">
                <flexbox-item class="details_right">
                  <span>食用油 * 2</span>
                  <span style="float: right;">&yen100</span>
                </flexbox-item>
              </flexbox>
            </group>
          </div>
...
<script>
  import { ViewBox, XHeader, Search, Flexbox, FlexboxItem, Scroller, XInput, XButton, TransferDom, Popup, Group } from 'vux'
  import $ from 'jquery'
  export default {
    directives: {
      TransferDom
...
//        console.log(scrollTop)
        setTimeout(function () {
          outer.css({ transform: 'translate3d(0,' + (endTop - top) + 'px,0)' })
          inner.css({ transform: 'translate3d(' + (endLeft - left) + 'px,0,0)' })
